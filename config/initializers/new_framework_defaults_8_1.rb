...
###
# Enable YJIT only outside of development and test.
#
# Not emitted by `rails app:update` (the generated 8.1 template omits it), but
# `load_defaults "8.1"` does set it: 7.2 turned YJIT on everywhere via `self.yjit = true`,
# and 8.1 narrows that to `self.yjit = !Rails.env.local?`. Development and test reload code
# and redefine methods (mocking), where YJIT is not generally faster.
#
